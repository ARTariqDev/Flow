# Flow: Your All-in-One Planner App

An all-in-one planner app allowing you to set timers, events, alarms, or make a to-do list to manage your day-to-day "Flow".

## Currently Implemented Features

- **Calendar** (Events have yet to be added)
- **Timers**
- **Alarms**
- **Stopwatch**
- **To-Do List**

## Remaining Features for the App

- **Calendar** (with the ability to add Events)
- **Dynamic Home Screen**: Displays any upcoming events, your current/remaining to-dos, any upcoming alarms, active timers, or an active stopwatch.
